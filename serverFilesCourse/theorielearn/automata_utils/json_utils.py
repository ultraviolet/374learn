...
def convert_states_for_json(
    ...
    if duplicated_names:
        raise JsonValidationError(duplicated_names, None, "Duplicate state names:")
...
def nfa_dump_json(nfa: NFA) -> NFAJsonDict:
    ...
    json_states = sorted(state_map[state] for state in nfa.states)
    json_input_symbols = sorted(nfa.input_symbols)
    ...
    json_transitions = {
        ...
        for start_state, transition in nfa.transitions.items()
    }

    json_initial_state = state_map[nfa.initial_state]
    json_final_states = sorted(state_map[state] for state in nfa.final_states)
