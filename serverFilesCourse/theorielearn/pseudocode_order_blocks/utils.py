...
def extract_submitted_pseudocode(
    data: Dict[str, Any], answer_name: str
) -> Optional[List[str]]:
    """
    ...
    if answer_name not in data["submitted_answers"]:
        return None
    return [
        html.unescape(line["inner_html"])
        for line in data["submitted_answers"][answer_name]
    ]


def submit_translated_code(
    data: Dict[str, Any],
    pseudocode: List[str],
    ...
    data["submitted_answers"]["_files"] = [
        {"name": "user_code.py", "contents": base64_encode_string("\n".join(code))}
    ]
