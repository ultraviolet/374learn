...
def generate_pl_graph(
    G: Union[nx.DiGraph, nx.Graph], label: Optional[str] = None, rankdir: str = "LR"
) -> str:
    """
...
def traveling_salesman_dp_optimized_helper(
    G: nx.Graph,
) -> Tuple[int, TSPSolutionsDictT]:
    ...
    for subs in chain.from_iterable(combinations(notStart, k) for k in range(2, lenG)):
        ...
        for end in subs:
            ...
            old_set_mem_arr[end] = min(
                new_set_mem_arr[end2] + distances[end2] for end2 in subs if end2 != end
            )

    solutions_dict = solutions[subset_to_bitstring(notStart)]

    return min(
        solutions_dict[node] + distances_from_start[node] for node in notStart
    ), solutions
...
def traveling_salesman_brute_force(
    G: nx.Graph, start_node: int = 0
) -> Tuple[List[int], int]:
    ...
    for tour_tuple in permutations(nodes):
        ...
        if cheapest_tour_weight is None or cheapest_tour_weight > tour_weight:
            cheapest_tour = tour
...
def find_hamiltonian_cycle(G: nx.Graph) -> Optional[List]:
    ...
    for potential_cycle in permutations(list(G)):
        edges = list(zip(potential_cycle, potential_cycle[1:])) + [
            (potential_cycle[0], potential_cycle[-1])
        ]
        if all(G.has_edge(first, second) for first, second in edges):
            return edges
    return None
...
def draw_graph_counterexample(
    ...
    left_edge_colors = [
        "red" if (u, v) in left_highlight or (v, u) in left_highlight else "black"
        for (u, v) in left_G.edges()
    ]
    left_edge_widths = [
        3.0 if (u, v) in left_highlight or (v, u) in left_highlight else 1.0
        for (u, v) in left_G.edges()
    ]
    right_edge_colors = [
        "red" if (u, v) in right_highlight or (v, u) in right_highlight else "black"
        for (u, v) in right_G.edges()
    ]
    right_edge_widths = [
        3.0 if (u, v) in right_highlight or (v, u) in right_highlight else 1.0
        for (u, v) in right_G.edges()
    ]

    nx.draw_circular(
        left_G,
        labels={n: n for n in left_G.nodes()},
        node_size=600,
        node_color="skyblue",
        edge_color=left_edge_colors,
    ...
    nx.draw_circular(
        right_G,
        labels={n: n for n in right_G.nodes()},
        node_size=600,
        node_color="sandybrown",
        edge_color=right_edge_colors,
        width=right_edge_widths,
        ax=right_ax,
    )
