...
def grade(data: pl.QuestionData) -> None:
    server_base.grade(data, lambda x: is_perfect_power(len(x), 2) and x == "0" * len(x))


def generate(data: Dict[str, Any]) -> None:
    data["params"]["non_regular_language"] = r"\{0^{n^{2}} \mid n \geq 0\}"
    data["params"]["fooling_set"] = r"\{0^{n^{2}} \mid n \geq 0\}"
    [fooling_set_member_1, fooling_set_member_2] = random.sample(
        ["0" * (n**2) for n in range(10)], 2
    )
