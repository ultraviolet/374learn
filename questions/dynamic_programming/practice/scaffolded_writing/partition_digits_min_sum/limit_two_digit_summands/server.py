...
def num_two_digit_handler(submission: sw_du.DPStudentSubmission) -> str:
    ...
    return f"if we decide to group the {first_or_last} two digits into a 2-digit term, then we would need to enforce that the rest of the summation uses at least <b><em>(t-1)</em></b> 2-digit terms"
...
def grade(data: QuestionData) -> None:
    ...
    grader.add_constraint(
        sw_du.can_compute_final_answer_constraint(
            ...
            feedback_elaboration="at least t 2-digit terms are used",
        ),
        0.4,
    )
    grader.add_constraint(
