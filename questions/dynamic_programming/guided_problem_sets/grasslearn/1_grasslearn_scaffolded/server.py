...
from theorielearn.scaffolded_writing.dp_cfgs import get_grasslearn_cfg
...
def streak_length_reduces_constraint(
    field_requiring_parameters: str,
) -> sw_du.ConstraintT:
    def constraint(submission: sw_du.DPStudentSubmission) -> str | None:
        def streak_length_reduces_handler(submission: sw_du.DPStudentSubmission) -> str:
        ...
        feedback = sw_du.reduces_recursively_constraint(
            field_requiring_parameters, streak_length_reduces_handler
        )(submission)
        if feedback:
            return feedback
        if (
            submission.does_path_exist("PREFIX_SUBPROBLEM")
            and submission.does_path_exist("STARTING_OR_ENDING", "starting")
        ) or (
            submission.does_path_exist("SUFFIX_SUBPROBLEM")
            and submission.does_path_exist("STARTING_OR_ENDING", "ending")
        ):
            return streak_length_reduces_handler(submission)
...
def generate(data: QuestionData) -> None:
    data["params"]["subproblem_definition_cfg"] = get_grasslearn_cfg().to_json_string()


def grade(data: QuestionData) -> None:
    grader = IncrementalConstraintGrader(
        sw_du.DPStudentSubmission, get_grasslearn_cfg()
    )
    ...
    grader.add_constraint(
        streak_length_reduces_constraint(field_requiring_parameters="STREAK_LENGTH"),
        0.7,
    )
