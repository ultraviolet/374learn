...
from theorielearn.shared_utils import sized_powerset
        ...
        for edge_set in sized_powerset(combinations(range(n), 2), min_size=1):
            ...
            try:
                ...
                if bool(cycle) != bool(path):
                    ...
                    plt.suptitle("Counterexample: Expand test results for details\n")
                    save_plot(plt)

                    if bool(path):
                        Feedback.add_feedback(
                            "Your reduction does not work for the example shown above.\n"
