...
TRANSFORMATION_NAME = "INSERT10"

TRANSFORMATION_DEFINITION = r"""
Given a language $L \subseteq \{0,1\}^*$, we define
$$\mathsf{INSERT10}(L) := \{ x10y \mid xy \in L \}.$$
"""

DESCRIPTION_OF_STATES = r"""
Intuitively, as $M'$ reads the input string, it will feed the characters to a simulation of $M$. However, $M'$ will *non-deterministically* choose a single `10` substring to omit from the simulation. Keeping this intuition in mind, we will now make the description more formal.

Every state $q$ in the DFA $M$ will correspond to three states in $M'$, as described below:

- The state $(q, \varepsilon)$ means that the simulation of $M$ is in state $q$ and $M'$ has not read any part of the inserted `10`.
- The state $(q, 1)$ means that the simulation of $M$ is in state $q$ and $M'$ has just read the `1` from the inserted `10`.
- The state $(q, 10)$ means that the simulation of $M$ is in state $q$ and $M'$ has already read and omitted the inserted `10`.
"""
...
def construct_M_prime(M: DFA) -> NFA:
    ...
    for q in M.states:
        transitions[(q, BEFORE)]["1"] = {(q, BETWEEN), (M.transitions[q]["1"], BEFORE)}
        transitions[(q, BEFORE)]["0"] = {(M.transitions[q]["0"], BEFORE)}

        transitions[(q, BETWEEN)]["1"] = set()
        transitions[(q, BETWEEN)]["0"] = {(q, AFTER)}
