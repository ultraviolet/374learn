...
TRANSFORMATION_NAME = "DELETE1STAR"

TRANSFORMATION_DEFINITION = r"""
Given a language $L \subseteq \{0,1\}^*$, we define
$$L' = \{xy \mid x \cdot 1^n \cdot y \in L ~\text{for some}~ n \ge 0\}.$$

In other words, $L'$ consists of all strings that can be formed by removing some (possibly empty) sequence of consecutive 1s from a string in $L$.
"""

DESCRIPTION_OF_STATES = r"""
Intuitively, as $M'$ reads the input string, it will feed the characters to a simulation of $M$. However, $M'$ will *non-deterministically* choose to insert an arbitrary number of consecutive 1s into the simulation at some point. Keeping this intuition in mind, we will now make the description more formal.

Every state $q$ in the DFA $M$ will correspond to three states in $M'$, as described below:

- The state $(q, \text{before})$ means that the simulation of $M$ is in state $q$ and $M'$ has not yet started inserting the 1s.
- The state $(q, \text{inserting1s})$ means that the simulation of $M$ is in state $q$ and $M'$ is currently inserting 1s into the simulation (it may insert more, or decide to stop).
- The state $(q, \text{after})$ means that the simulation of $M$ is in state $q$ and $M'$ has finished inserting the sequence of 1s.
"""

BEFORE = "before"
INSERTING1S = "inserting1s"
AFTER = "after"
STATE_LABELS = [BEFORE, INSERTING1S, AFTER]
...
def should_use_dfa(M: DFA) -> bool:
    ...
    This ensures students understand that 1s are being inserted into the simulation of M.
...
def construct_M_prime(M: DFA) -> NFA:
    ...
    for q in M.states:
        # From before state:
        # Use epsilon to transition to inserting1s state (start inserting)
        transitions[(q, BEFORE)][""] = {(q, INSERTING1S)}
        ...
        # Use epsilon to either insert another 1 or stop inserting
        transitions[(q, INSERTING1S)][""] = {
            (M.transitions[q]["1"], INSERTING1S),  # Insert another 1
            (q, AFTER),  # Stop inserting
        }
