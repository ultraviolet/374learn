...
TRANSFORMATION_NAME = "INV_THIRDS"

TRANSFORMATION_DEFINITION = r"""
Given a language $L \subseteq \{0,1\}^*$, we define
$$L' = \mathit{thirds}^{-1}(L) := \{w \in \Sigma^* \mid \mathit{thirds}(w) \in L\}$$
...
Intuitively, as $M'$ reads the input string, it needs to track which position it's at (mod 3) and simulate $M$ only on symbols at positions that are multiples of 3. The NFA $M'$ will feed symbols to $M$ only at positions 3, 6, 9, etc.

Every state $q$ in the DFA $M$ will correspond to three states in $M'$, as described below:

- The state $(q, 0)$ means that the simulation of $M$ is in state $q$ and $M'$ has just read a symbol at a position that is a multiple of 3 (positions 3, 6, 9, ...).
- The state $(q, 1)$ means that the simulation of $M$ is in state $q$ and $M'$ is at position $\equiv 1 \pmod{3}$ (positions 1, 4, 7, ...).
- The state $(q, 2)$ means that the simulation of $M$ is in state $q$ and $M'$ is at position $\equiv 2 \pmod{3}$ (positions 2, 5, 8, ...).
"""

POS_0 = 0  # Just read position 3, 6, 9, ... (multiple of 3)
...
def construct_M_prime(M: DFA) -> NFA:
    ...
    # Accept in any state where M accepts, at any position
